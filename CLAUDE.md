...
| **13 static pages built** | `app/{about,blog,careers,case-studies,help,updates,docs,privacy,terms,cookies,compliance}/page.tsx` + `ContentPageTemplate.tsx`; `app/login` + `LoginPageClient.tsx`; `app/demo` + `DemoPageClient.tsx` | New reusable bilingual `ContentPageTemplate` (hero + prose sections + card grid + CTA band). `/login` is a sign-in form UI (no backend). `/demo` auto-opens the demo modal. |
| **Fixed broken Features→Channel Manager menu links** | `Navbar.tsx` | The 8 brand items (AQAR, Ejar, Airbnb, Booking, AGODA, Google VR, Qotoon, Attiude) had slugs `channel-manager/<x>` rendered as `/features/channel-manager/<x>` — two-segment paths that 404 (the `[slug]` route only matches one segment). Added an explicit `href` per item pointing to the real `/integrations/<slug>` page; both desktop & mobile renderers now use `item.href ?? /features/${slug}`. All 8 now resolve to prerendered integration pages. |

## 6d. Changes Made in Session (Jun 15, 2026 — pt 2)

| Change | File(s) | Details |
|---|---|---|
| **Fixed demo modal reopening after close** | `lib/DemoModalContext.tsx`, `components/sections/DemoPageClient.tsx` | Root cause: `openModal`/`closeModal` were recreated every provider render, so `/demo`'s auto-open `useEffect` (dep `[openModal]`) re-ran whenever `isOpen` changed — closing the modal reopened it in a loop. Memoized the context functions/value with `useCallback`+`useMemo` so the effect runs once and close stays closed. (A `useRef` guard was tried first but broke auto-open under React Strict Mode double-invoke — reverted.) |
